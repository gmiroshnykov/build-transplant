...
exports.lookup = function(repository, revset, callback) {
  ...
  $.ajax(url, {data: data}).success(function(data, status, xhr) {
...
React.renderComponent(TransplantApp({repositories: REPOSITORIES}), document.getElementById('app'));
...
var Alert = React.createClass({
  displayName: 'Alert',
  propTypes: {alert: React.PropTypes.shape({
  ...
  render: function() {
    ...
    }, React.DOM.span({'aria-hidden': "true"}, "×"), React.DOM.span({className: "sr-only"}, "Close")), alert.message));
  }
});
...
var Alerts = React.createClass({
  displayName: 'Alerts',
  propTypes: {
    items: React.PropTypes.arrayOf(React.PropTypes.shape({id: React.PropTypes.any.isRequired})),
  ...
  render: function() {
    var $__0 = this;
    var alerts = _.map(this.props.items, (function(item) {
      return Alert({
        key: item.id,
        alert: item,
        onClose: _.partial($__0.props.onDismiss, item.id)
      });
    }));
    return (React.DOM.div({className: "alerts"}, alerts));
  }
});
...
var EditableCommit = React.createClass({
  displayName: 'EditableCommit',
  propTypes: {commit: React.PropTypes.shape({
  ...
  render: function() {
    ...
    return (React.DOM.form({role: "form-horizontal"}, React.DOM.div({className: "form-group"}, this.renderLabel('ID'), this.renderStatic(React.DOM.code(null, commit.node))), React.DOM.div({className: "form-group"}, this.renderLabel('Author'), this.renderStatic(author)), React.DOM.div({className: "form-group"}, this.renderLabel('Date'), this.renderStatic(date)), React.DOM.div({className: "form-group clearfix"}, this.renderLabel('Message'), React.DOM.div({className: "col-sm-11"}, React.DOM.textarea({
  ...
  renderLabel: function(text) {
    return React.DOM.label({className: "control-label col-sm-1"}, text);
  },
  renderStatic: function(text) {
    return React.DOM.p({className: "form-control-static col-sm-11"}, text);
  }
});
...
var RepositoriesListField = React.createClass({
  displayName: 'RepositoriesListField',
  propTypes: {
  ...
  render: function() {
    var repositories = [React.DOM.option({
      key: "",
      value: ""
    })];
    _.each(this.props.repositories, function(repository) {
      repositories.push(React.DOM.option({
        key: repository.name,
        value: repository.name
      }, repository.name, " (", repository.path, ")"));
    });
    return (React.DOM.div({className: "form-group"}, React.DOM.label({
      htmlFor: this.props.name,
      className: "control-label"
    }, this.props.title), React.DOM.select({
...
var RevsetField = React.createClass({
  ...
  render: function() {
    ...
    return (React.DOM.div({className: "form-group"}, React.DOM.label({htmlFor: "revset"}, "Revset"), React.DOM.div({className: "input-group"}, React.DOM.input({
      ref: "revset",
      placeholder: "Revset",
      type: "text",
      value: value,
      ...
      onChange: this.handleChange,
      className: "form-control"
    }), React.DOM.span({className: "input-group-btn"}, React.DOM.button({
...
var TransplantApp = React.createClass({
  ...
  handleTransplant: function() {
    ...
    var items = [];
    revsets.forEach((function(revset) {
      if (revset.squash) {
        items.push({
        ...
        revset.commits.forEach((function(commit) {
          items.push({
            commit: commit.node,
    ...
    return Api.transplant(source, target, items, (function(err, result) {
      $__0.setState({inProgressTranslant: false});
      if (err) {
        var error = {message: err.message};
        if (err.details) {
          error.details = "command: " + err.details.cmd.join(' ') + "\n" + "returncode: " + err.details.returncode + "\n" + "stdout: " + err.details.stdout + "\n" + "stderr: " + err.details.stderr + "\n";
        }
        return $__0.setState({
          transplantDone: false,
          transplantResult: {error: error}
        });
      }
      return $__0.setState({
        transplantDone: true,
  ...
  renderButton: function() {
    return !this.state.transplantDone ? this.renderTransplantButton() : this.renderResetButton();
  },
  renderTransplantButton: function() {
    ...
    return (React.DOM.button({
      ...
      className: "btn btn-primary btn-lg col-md-2 pull-right"
    }, text));
  },
  renderResetButton: function() {
    ...
    return (React.DOM.button({
      ...
      className: "btn btn-warning btn-lg col-md-2 pull-right"
    }, "Reset form"));
  },
  ...
  renderError: function() {
    ...
    var details = result.error.details ? React.DOM.div({className: "panel-body"}, React.DOM.pre(null, result.error.details)) : null;
    return (React.DOM.div({className: "panel panel-danger"}, React.DOM.div({className: "panel-heading"}, result.error.message), details));
  },
  render: function() {
    ...
    var resultRow = null;
    if (result || button) {
      resultRow = React.DOM.div({className: "row"}, React.DOM.div({className: "form-group clearfix"}, result, button));
    }
    var errroRow = null;
    if (error) {
      errroRow = React.DOM.div({className: "row"}, error);
    }
    return (React.DOM.div(null, React.DOM.div({className: "row"}, React.DOM.h1(null, "Transplant")), React.DOM.div({className: "row"}, TransplantForm({
    ...
    })), React.DOM.div({className: "row"}, TransplantRevsets({
      revsets: this.state.revsets,
...
var TransplantForm = React.createClass({
  displayName: 'TransplantForm',
  propTypes: {
    repositories: React.PropTypes.arrayOf(React.PropTypes.shape({name: React.PropTypes.string.isRequired})),
    sourceRepository: React.PropTypes.string.isRequired,
    targetRepository: React.PropTypes.string.isRequired,
    revsets: React.PropTypes.arrayOf(React.PropTypes.shape({revset: React.PropTypes.string.isRequired})),
  ...
  handleAddRevset: function(revset) {
    ...
    return Api.lookup(sourceRepository, revset, (function(err, result) {
      $__0.setState({inProgress: false});
      if (err) {
        $__0.alertAdd('danger', err, 'error');
        return;
      }
      $__0.refs.revsetField.reset();
      result.revset.revset = revset;
      $__0.props.onAddRevset(result.revset);
    }));
  },
  lastAlertId: 0,
  ...
  render: function() {
    ...
    var sourceRepositories = targetRepository == '' ? this.props.repositories : _.reject(this.props.repositories, _.matches({name: targetRepository}));
    var targetRepositories = sourceRepository == '' ? this.props.repositories : _.reject(this.props.repositories, _.matches({name: sourceRepository}));
    var canAdd = !!sourceRepository;
    return (React.DOM.form({
      className: "transplantForm",
      role: "form"
...
var TransplantRevset = React.createClass({
  ...
  propTypes: {
    revset: React.PropTypes.shape({
      ...
      commits: React.PropTypes.arrayOf(React.PropTypes.shape({node: React.PropTypes.string.isRequired}))
    }),
  ...
  render: function() {
    ...
    return (React.DOM.div({className: "panel panel-default"}, React.DOM.div({className: "panel-heading clearfix"}, React.DOM.div({className: "pull-left"}, "Revision Set ", React.DOM.code(null, revset.revset), " (commits: ", commitsCount, ")"), React.DOM.div({
      className: "form-inline pull-right",
      role: "form"
    }, this.renderSquashCheckbox(), React.DOM.button({
      type: "button",
      className: "btn btn-danger btn-xs",
      onClick: this.props.onDelete
    }, "delete"))), React.DOM.div({className: "list-group"}, revset.squash ? this.renderSquashedForm() : this.renderCommits())));
  },
  renderSquashCheckbox: function() {
    ...
    return (React.DOM.div({
      ...
      onChange: this.handleChangeSquash
    }), " squash")));
  },
  renderSquashedForm: function() {
    ...
    return (React.DOM.div({className: "list-group-item"}, React.DOM.form({role: "form"}, React.DOM.div({className: "form-group"}, React.DOM.label(null, "Squashed message"), React.DOM.textarea({
      className: "form-control",
      rows: "5",
  ...
  renderCommits: function() {
    var $__0 = this;
    return this.props.revset.commits.map((function(commit) {
      return React.DOM.div({
        key: commit.node,
        className: "list-group-item"
      }, EditableCommit({
        commit: commit,
        onChange: _.partial($__0.props.onChangeCommit, commit.node)
      }));
    }));
  }
});
...
var TransplantRevsets = React.createClass({
  ...
  render: function() {
    var $__0 = this;
    return (React.DOM.div({className: "transplantRevsets"}, this.props.revsets.map((function(revset) {
      return TransplantRevset({
        key: revset.revset,
        revset: revset,
        onChangeSquash: _.partial($__0.props.onChangeSquash, revset.revset),
        onChangeCommit: _.partial($__0.props.onChangeCommit, revset.revset),
        onChangeSquashedMessage: _.partial($__0.props.onChangeSquashedMessage, revset.revset),
        onDelete: _.partial($__0.props.onDelete, revset.revset)
      });
    }))));
  }
});
